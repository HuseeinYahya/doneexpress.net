...
/**
 * // DOMContentLoaded(fixed-top)
 */
document.addEventListener("DOMContentLoaded", function(){
el_fixed_top = document.querySelector('.fixed-top');
...
if(el_fixed_top){
  ...
  window.addEventListener('scroll', function() {
        ...
        if(scroll_top < last_scroll_top) {
            el_fixed_top.classList.remove('scrolled-down');
            el_fixed_top.classList.add('scrolled-up');
        }
        else {
            if (window.scrollY > 80) {
                el_fixed_top.classList.remove('scrolled-up');
                el_fixed_top.classList.add('scrolled-down');
            }
        }
...
/**
 * Order tracking offcanvas, tracking input validation, and contact link
 */
document.addEventListener('DOMContentLoaded', function () {
const input = document.getElementById('searchOrder');
const cBtn = document.getElementById('trackBtn');
const offcanvasElement = document.getElementById('orderOffcanvas');
...
trackBtn.addEventListener('click', function (e) {
  ...
  document.getElementById('orderNumber1').textContent = `${value}`;
  document.getElementById('orderNumber2').textContent = `${value}`;
  document.getElementById('whatsappLink').href = `https://wa.me/967780050504?text=مرحبًا%20أرغب%20في%20معرفة%20المزيد%20عن%20تفاصيل%20الطلب%20الخاص%20بي%20رقم:%20${value}`;
  bsOffcanvas.show();
...
/**
 * Lenis Enhance smooth scrolling by using lenis
 */
...
document.addEventListener('shown.bs.offcanvas', (e) => {
  const off = e.target
  off.setAttribute('data-lenis-prevent', '')
  off.style.overflowY = 'auto'
  off.style.webkitOverflowScrolling = 'touch'
  off.style.touchAction = 'auto'
  lenis.stop()
...
document.addEventListener('hidden.bs.offcanvas', (e) => {
  const off = e.target
  off.removeAttribute('data-lenis-prevent')
  off.style.overflowY = ''
  off.style.touchAction = ''
  lenis.start()
  document.html.style.overflow = ''
})

/**
 * scrollTrigger for .scrollTrigger1 & .scrollTrigger1
 */
gsap.registerPlugin(ScrollTrigger);
// --- scrollTrigger1 PANEL ---
gsap.from(".line-1", {
  scrollTrigger: {
...
const gProgressBars = document.querySelectorAll('.progress-bar');
const gTimelineBars = document.querySelectorAll('.timeline-bar');
gProgressBars.forEach((progressBar, index) => {
  if (!progressBar.closest('.past')) {
    const timelineBar = gTimelineBars[index];
    if (timelineBar) {
      gsap.to(progressBar, {
        scrollTrigger: {
          trigger: timelineBar,
...
/**
 * Animation behavior for Track Order Button #searchOrderInput
 */
const myClick = document.querySelectorAll('.searchClick')
myClick.forEach(el => {
const myInput = document.getElementById('searchOrder')
const myOrderInput = document.getElementById('searchOrderInput')
...
el.addEventListener('click', () => {
  ...
  myOrderInput.classList.add('blink-border');
  setTimeout(() => {
    myOrderInput.classList.remove('blink-border');
  }, 1500);
})})


const showBtn = document.querySelector('.showAllServicesBtn');
const toggle = document.getElementById('servicesMenu');
const servicesDropdown = new bootstrap.Dropdown(toggle);
const servicesMenuContent = document.getElementById('servicesMenuContent');

const navbarCollapseEl = document.getElementById('navbarNav');
const navbarCollapse = navbarCollapseEl ? new bootstrap.Collapse(navbarCollapseEl, { toggle: false }) : null;

showBtn.addEventListener('click', async () => {

  // في الموبايل: افتح collapse إذا كان مغلقًا
  if (navbarCollapseEl && !navbarCollapseEl.classList.contains('show')) {
    navbarCollapse.show();
    await new Promise(r => setTimeout(r, 220)); // انتظار انتهاء الأنيميشن
  }
  ...
  setTimeout(() => {
    ...
    if (navbarCollapseEl && navbarCollapseEl.classList.contains('show')) {
      setTimeout(() => navbarCollapse.hide(), 150);
    }
  }, 4000);
  ...
  if (!servicesMenu.classList.contains('show')) {
    ...
    setTimeout(() => {
      servicesMenuContent.classList.add('blink-border');

      setTimeout(() => {
        servicesMenuContent.classList.remove('blink-border');
      }, 4000);
